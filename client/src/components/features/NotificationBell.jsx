  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="p-2 rounded-full hover:bg-white/5 text-white/80 relative">
        <Bell size={20} />
        {pendingCount > 0 && (
          <span className="absolute top-0.5 right-0.5 w-4 h-4 bg-red-500 rounded-full text-2xs font-bold text-white flex items-center justify-center">
            {pendingCount}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-[#111111] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
          <div className="p-3 border-b border-white/5 flex items-center justify-between">
            ...
            <button onClick={() => { setOpen(false); navigate('/admin/orders'); }} className="text-2xs text-accent hover:text-accent/80">View All</button>
          </div>
              ...
              orders.map((o, i) => (
                <div key={i} className="px-3 py-2.5 border-b border-white/[0.03] hover:bg-white/[0.02] transition-all cursor-pointer" onClick={() => { setOpen(false); navigate('/admin/orders'); }}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-2xs font-semibold uppercase ${statusColor(o.status)}`}>{o.status}</span>
                  </div>
                  <div className="flex items-center justify-between mt-0.5">
                    <span className="text-2xs text-white/40">{o.customer}</span>
                    <span className="text-2xs font-semibold text-white/60">₹{(o.total || 0).toLocaleString('en-IN')}</span>
                  </div>
                </div>
              ))
            )}
